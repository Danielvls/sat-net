...
for i, file_path in enumerate(file_paths):
    ...
    mean_total_cost = data['Mean Total Cost']  # Corrected column name
    std_total_cost = data['Std Dev Total Cost']  # Corrected column name

    # Plot with different colors, marker styles, and individual labels
    plt.errorbar(avg_flow_nums, mean_total_cost, yerr=std_total_cost,
...
plt.ylabel('Total Cost', fontsize=14)  # Updated label to match the data
plt.legend()
plt.grid(True)

# Save the plot with reduced white space
plt.savefig(project_root / 'plot' / 'total_cost.svg', format='svg', bbox_inches='tight')
