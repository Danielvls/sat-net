...
for i, file_path in enumerate(file_paths):
    ...
    mean_link_usage = data['Mean Link Usage']  # Corrected column name
    std_link_usage = data['Std Dev Link Usage']  # Corrected column name

    # Plot with different colors, marker styles, and individual labels
    plt.errorbar(avg_flow_nums, mean_link_usage, yerr=std_link_usage,
...
plt.ylabel('Link Usage(%)', fontsize=14)  # Updated label to match the data
plt.legend()
plt.grid(True)

# Save the plot with reduced white space
plt.savefig(project_root / 'plot' / 'link_usage.svg', format='svg', bbox_inches='tight')
